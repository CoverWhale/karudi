...
def find_csv_files(base_path):
    """Find CSV files within the nested migration directory structure."""
    csv_files = []
    for root, _, files in os.walk(base_path):
        if "data/python/migration/output" in root:  # Ensure correct subdirectory
            for file in files:
...
def extract_data_model(file_name):
    """Extract the data model type (WIP, Quotes, Policy) from filename."""
    if "WIP" in file_name:
        return "WIP"
    elif "Quotes" in file_name:
        return "Quotes"
...
def process_csv_files(extracted_paths):
    ...
    for model_type, subsets in data_model_groups.items():
        ...
        for shared_subset, file_list in subsets.items():
            ...
            dfs = []  # Store DataFrames for this subset
            ...
            # Ensure data exists before writing output
            if dfs:
                combined_df = pd.concat(dfs, ignore_index=True)
                output_file = os.path.join(PROCESSED_DIR, f"{shared_subset}.csv")
...
def main():
    ...
    zip_files = get_latest_zip_files(drive_service, GOOGLE_DRIVE_FOLDER_ID)

    if not zip_files:
        logging.info("No matching zip files found.")
        return

    logging.info(f"Processing ZIP files for the latest available date.")
    ...
    output_files = process_csv_files(extracted_paths)
